...
from preprocess import read_entity_from_id, read_relation_from_id, init_embeddings, build_data, generate_noise_data
...
def parse_args():
    ...
    # [please ignore] arguments for generate noise
    args.add_argument("-g_noise", "--get_noise",
                      default=False, help="generate noise") 
    args.add_argument("-u_noise", "--use_noise",
                      default=False, help="use noise dataset and algorithm")     
    args.add_argument("-n_percent", "--noice_percent",
                  default=0.1, help="noice pertentage of raw dataset")
    args.add_argument("-gm", "--gamma", default=0.1,  help="gama of triple quality")
    args.add_argument("-eta", "--eta", default=0.95,  help="penalty factor bad triple")
    args.add_argument("-eps", "--epsilon", default=0.05,  help="award factor of good triple")
    
    # argument for entityrank
    args.add_argument("-d", "--d", default=0.85,  help="factor in entity rank")
...
def load_data(args):
    train_data, validation_data, test_data, entity2id, relation2id, headTailSelector, unique_entities_train = build_data(
        args.data, args.use_noise, args.noice_percent, is_unweigted=False, directed=True)
...
if(args.get_noise): 
    print('Generate noise dataset of: '+ args.data)
    starttime = datetime.datetime.now()
    generate_noise_data(args.data, args.noice_percent)
    endtime = datetime.datetime.now()
    print(args.data+'-'+str(int(args.noice_percent*100))+'%.txt has been generated\nTime:'+str((endtime - starttime).seconds))
...
def train_gat(args):
    ...
    model_gat = SpKBGATModified(entity_embeddings, relation_embeddings, args.entity_out_dim, args.entity_out_dim,
                                args.drop_GAT, args.alpha, args.gamma, args.eta, args.epsilon, args.d, args.nheads_GAT)
    ...
    # [Please ignore] initialize confidence
    if(args.use_noise):
        confidence = nn.Parameter(torch.zeros(size=(1, len(Corpus_.train_triples))))
        nn.init.xavier_uniform_(confidence, gain=1.414)
        confidence = confidence.data.numpy().tolist()[0]
        
    #initialize entity rank
    #There are many methods for initializing entity rank, in practice we use the following method 
    entity_rank = nn.Parameter(torch.zeros(size=(1, len(entity_embeddings))))
    nn.init.xavier_uniform_(entity_rank, gain=1.414)
    entity_rank = entity_rank.data.numpy().tolist()[0]
    entity_rank = list(map(abs, entity_rank))    
#    entity_rank = np.random.uniform(0, 1, len(entity_embeddings))
    
    if CUDA:
        entity_rank = torch.FloatTensor(entity_rank).cuda()
    else:
        entity_rank = torch.FloatTensor(entity_rank)
    ...
    for epoch in range(args.epochs_gat):
        ...
        for iters in range(num_iters_per_epoch):
            ...
            if CUDA:
                ...
                # [Please ignore]
                if(args.use_noise):
                    confidence = torch.FloatTensor(confidence).cuda()
            ...
            if(args.use_noise):
                entity_embed, relation_embed = model_gat(
                    Corpus_, Corpus_.train_adj_matrix, train_indices, current_batch_2hop_indices, confidence, entity_rank)
            else:
                entity_embed, relation_embed = model_gat(
                    Corpus_, Corpus_.train_adj_matrix, train_indices, current_batch_2hop_indices, False, entity_rank)
